...
18. `request.security_lower_tf(..., ignore_invalid_symbol=True)` tuple
    expression capture 已准备好，等待 TradingView CSV 导出；Pine 侧脚本对
    invalid lower-TF 返回的 `na` array id 先做 guard，再观测每个 tuple slot
    的空数组/fallback 语义。

推荐的下一个最小切片：

1. 先导出当前 lower-TF invalid-symbol tuple ignore capture。
