...
20. `request.security(..., ignore_invalid_symbol=True)` calculated expression
    capture 已准备好，等待 TradingView CSV 导出；继续覆盖 ignored invalid
    symbol 进入 callable expression / requested-expression 计算路径后的
    `na` / fallback 语义。

推荐的下一个最小切片：

1. 先导出当前 invalid-symbol expression ignore capture。
