...
22. `request.security_lower_tf()` requested-context capture 已完成本地语义和
    golden scaffold，等待 TradingView CSV 导出；继续覆盖 lower-TF 内部
    `time` / `time_close`、`timeframe.multiplier` 与 provider bar slot 重建。

推荐的下一个最小切片：

1. 先导出当前 lower-TF requested-context capture。
