# Pyne Runtime 功能补全与宿主验收执行计划

> [!IMPORTANT]
> **状态：当前执行计划。** 本文面向 `0.3.x` 候选之后的功能补全与 CandleScope
> 宿主落地。当前已经实现的能力、明确边界与证据，始终以
...
Pine API 名称数量，而是优先关闭真实 CandleScope 工作流中的阻塞项，并让每一项新增
能力都有可重复的语义、性能、恢复和宿主证据。

## 0. 最终目标

本计划完成后，应得到以下结果：

1. 当前 `0.3.x` Runtime 能以候选 wheel 形式通过独立安装和 CandleScope 宿主验收。
...
4. 外部 Pine 库只按固定 owner/library/version/member 增加适配器，未知成员继续
   fail closed。
5. 新 `request.*` 只有在宿主数据能力已经存在时才进入 Runtime，并同步完成 provider、
   schema、diagnostics、capture 和 host acceptance。
...
8. 任何能力进入“已支持”列表前，都必须经过源码路径、测试、文档、构建 wheel 和宿主
   路径五层验收。
...
| 本地全门禁 | 863 tests passed；性能、稳定性、build、Twine、installed-wheel smoke 通过 |
| CandleScope package focused tests | bridge + workbench 41 passed |
| CandleScope backend focused tests | workbench manifest、strategy provider、runtime architecture 10 passed |

测试数量会增长，后续验收不得把 `863`、`41` 或 `10` 写成必须相等的硬编码条件。
正确判据是命令退出码为 0、没有 skipped critical gate，并保存当次实际计数。

当前还有两项需要在第一个功能切片前处理的事实：

- 工作区存在与仓库所有者 URL 迁移相关的未提交修改。它们不属于功能补全切片，不得与
  TA/request/strategy 修改混合提交。
- 文档存在少量能力计数漂移，例如 Incremental 指南仍列旧的 27 个 helper，而公开能力
  契约已经是 39 个。Phase 1 会先把这种漂移变成自动门禁。
...
### 2.2 归 bridge/workbench 负责

- CandleScope SDK 映射、版本锁、chart context 和协议转换。
- Runtime 输出到 Host Render IR/chart layer 的显式投影。
- malformed/unsupported output 的 fail-closed 校验。
- v1 batch bridge 与 v2 workbench 的能力隔离。

### 2.3 归 CandleScope Host 负责
...
- 不在一个提交中同时修改 Runtime、bridge、workbench 和 Host 多个领域。
...
Phase 2 冻结代表性工作负载并生成需求榜
   ↓
Phase 3 验收当前 0.3 候选的 CandleScope 纵向链路
   ↓
...
- 如果影响 Host，CandleScope focused acceptance 通过。
...
- `$PyneScriptRoot`：已经迁移的 `.py`/`.pyne` 脚本，用于 `pyne inspect`。

执行者必须把下面占位符换成真实路径：

```powershell
$PineCorpusRoot = (Resolve-Path '<ABSOLUTE-PINE-CORPUS-DIRECTORY>').Path
$PyneScriptRoot = (Resolve-Path '<ABSOLUTE-PYNE-SCRIPT-DIRECTORY>').Path
...
if (-not (Test-Path -LiteralPath $PyneScriptRoot -PathType Container)) {
    throw "Missing Pyne script corpus: $PyneScriptRoot"
}
```
...
如果实际脚本使用 `.pyne` 扩展名，再独立执行一次 `--pattern '*.pyne'`，不要把多个扩展名
假设成一个 glob。
...
如果语料路径、授权或冻结版本无法确认，Phase 2 标记阻塞；不得凭当前 17 个 Batch-only
名称直接决定全部实现顺序。

## 8. Phase 3：验收当前 0.3 候选的 CandleScope 纵向链路

### 8.1 目标

在添加更多能力前，证明现有 `0.3.0rc2` 已能通过 bridge、workbench 和关键 Host 路径。
该阶段的失败优先作为集成缺口修复，而不是用新增 Runtime API 掩盖。

### 8.2 边界

- `candlescope-plugin-pyne` 保持冻结的 v1 batch `script-runtime/1`。
- session/data broker/native output v2 属于 `candlescope-plugin-pyne-workbench`。
- bridge 使用 `0.3.0.dev0` candidate，Runtime 使用 `0.3.0rc2` candidate。
- 不在本阶段修改 CandleScope 其他业务模块。

### 8.3 Source-focused 验收

使用 CandleScope 自己的 `.venv`，因为它包含 backend 与 workbench 的测试依赖：

```powershell
$CandleScopeRoot = 'E:\Disk0Merged\H\program\CandleScope'
$HostPython = Join-Path $CandleScopeRoot '.venv\Scripts\python.exe'
$SdkSrc = Join-Path $CandleScopeRoot 'packages\candlescope-plugin-sdk\src'
$BridgeRoot = Join-Path $CandleScopeRoot 'packages\candlescope-plugin-pyne'
$WorkbenchRoot = Join-Path $CandleScopeRoot 'packages\candlescope-plugin-pyne-workbench'
$PriorPynePath = $env:PYTHONPATH

try {
    $env:PYTHONPATH = @(
        (Join-Path $PyneRoot 'src')
        (Join-Path $CandleScopeRoot 'backend')
        $SdkSrc
        (Join-Path $BridgeRoot 'src')
        (Join-Path $WorkbenchRoot 'src')
    ) -join ';'

    & $HostPython -m pytest `
      (Join-Path $BridgeRoot 'tests') `
      (Join-Path $WorkbenchRoot 'tests') -q
    if ($LASTEXITCODE -ne 0) {
        throw 'Bridge/workbench source-focused acceptance failed.'
    }
}
finally {
    $env:PYTHONPATH = $PriorPynePath
}
```
...
$PriorPynePath = $env:PYTHONPATH
try {
    $env:PYTHONPATH = Join-Path $CandleScopeRoot 'backend'
    Push-Location (Join-Path $CandleScopeRoot 'backend')
    & $HostPython -m pytest `
      tests\test_pyne_workbench_manifest.py `
      tests\test_pyne_strategy_provider.py `
      tests\test_plugin_runtime_architecture.py -q
    if ($LASTEXITCODE -ne 0) {
        throw 'CandleScope Pyne backend acceptance failed.'
    }
}
finally {
    Pop-Location
...
随后按 bridge 的 `release-lock.candidate.json` 与 package version 规则验证候选 lock。不得把
candidate wheel URL 写进 immutable published `release-lock.json`，也不得覆盖 `0.2.0` 已发布
artifact。
...
| 场景 | 必须证明 |
| --- | --- |
| v1 batch | 老 `script-runtime/1` 路径不获得 session/data-broker 权限 |
| v2 batch | current chart + bounded bars + request broker + native output v2 |
| Incremental open/seed | 历史仅初始化一次，revision 一致 |
| Preview | preview state/object/strategy 不污染 committed 状态 |
| Confirmed bar | 状态、绘图、strategy ledger 只在确认后持久化 |
| Reconnect | snapshot/restore 后结果与原 session 等价 |
| Capacity/TTL | active 满载时 fail closed，idle session 可 LRU/TTL 回收 |
| Unsupported output | 不静默丢字段或伪装成其他 chart primitive |
| Request failure | provider 分类、请求坐标和 diagnostics 可观测 |
| Version mismatch | bridge/runtime/schema 不兼容时启动或打包失败 |

### 8.6 验收标准

- source-focused、candidate wheel、bridge/workbench、backend focused gates 全部通过。
- 失败能归因到 Runtime、bridge、workbench 或 Host 中唯一 owner。
- 当前能力纵向可用后，才允许把新 P0 API 加入候选。
...
9. 在 CandleScope bridge/workbench 实现数据 broker 映射。
10. 跑 Runtime full gate 与 Host focused acceptance。
...
- 对应 CandleScope data broker/adapter tests
...
2. 由 CandleScope sidecar 提供独立进程身份、文件系统、网络和系统调用边界。
...
- Phase 2 的 P0 阻塞项全部为 implemented、host-resolved 或明确 reject。
...
重复 Phase 3 的 bridge/workbench 41-test family 与 backend focused gate，并用候选 wheel
替代 source path 完成至少一次干净环境验收。最终报告必须明确：

- v1 bridge 验收结果；
- v2 workbench batch 结果；
- Incremental seed/preview/confirmed/reconnect 结果；
- request data-broker 结果；
- Render IR v2 支持和仍未映射的 collection；
- version mismatch 和 rollback 结果。
...
- 保留已发布 `0.2.0rc1` 和 immutable release lock。
- 候选失败时回退 Host candidate lock，不覆盖旧 artifact。
...
### Host validation
<commands and results or not applicable>
...
- 新功能迫使 core 联网或导入 CandleScope 私有模块；
...
3. **当前候选宿主验收切片**：保存 bridge/workbench/backend/candidate-wheel 证据，不新增
   Runtime API。
4. **第一个 Incremental TA 切片**：从需求榜最高项中选择一个 helper，按 Phase 4 完成
   red test、实现、restore、performance、docs、full gate 和 host script 验收。

完成这四步后再重新排序后续工作。若代表性脚本已经全部可运行，不应为了清空 17 项差集
继续机械扩张；应转向迁移体验、宿主可用性和候选发布闭环。
...
- candidate wheel 与 CandleScope 纵向链路通过；
