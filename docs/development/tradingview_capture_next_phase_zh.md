...
- 当前 capture 状态：17 captured、10 not_captured、0 missing。
- Priority case 总数：10。
...
- 第二批非 priority case 已开始：`cash_per_order_slippage_round_trip`、
  ...
  `intrabar_path_low_high_exit` 与 `oca_cancel_waits_for_intraday_risk_reset`
  已进入 parity gate。
...
priority 10 个完成后，继续把剩余 17 个 case 推进到 captured。当前剩余 10 个。
