...
| Strategy events | `strategy(...)`, `strategy.entry_when`, `strategy.order_when`, `strategy.close_when`, `strategy.close_all`, `strategy.exit(qty=...)` | Partial | Event and position replay layer only; no broker simulation, margin, pending order cancellation, or intrabar path | `tests/test_strategy_runtime.py` | `docs/api/strategy.md` |
...
- More formal fill models, margin, pending order cancellation, and intrabar path modeling.
