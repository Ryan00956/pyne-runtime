...
| Strategy events | `strategy.entry_when`, `strategy.close_when`, `strategy.exit(qty=...)`, `strategy.configure(pyramiding=...)` | Partial | Event and position replay layer only; no broker simulation, slippage, commission, or intrabar path | `tests/test_strategy_runtime.py` | `docs/api/strategy.md` |
