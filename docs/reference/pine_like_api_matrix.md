...
| Strategy events | `strategy(...)`, `strategy.entry_when`, `strategy.close_when`, `strategy.exit(qty=...)`, `strategy.configure(...)` | Partial | Event and position replay layer only; no broker simulation, margin, or intrabar path | `tests/test_strategy_runtime.py` | `docs/api/strategy.md` |
