...
| Strategy risk | `strategy.risk.allow_entry_in(strategy.direction.long)` | Partial | Entry-direction risk applies to `strategy.entry*`; lower-level `strategy.order*` remains an unrestricted net-position event API | `tests/test_strategy_runtime.py` | `docs/api/strategy.md` |
