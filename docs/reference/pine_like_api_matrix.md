...
| Collections | `array.new_float(2, 1.0)`, `array.push(a, close[1])`, `array.get(a, -1)`, `a.avg()`; `map.put(m, "fast", 10)`, `map.get(m, "fast")`, `map.keys(m)` | Supported | `array.from(...)` is exposed as `array.from_values(...)` / `array.from_list(...)` because `from` is a Python keyword; arrays/maps are Python objects, not Pine handles; `matrix.*` is still planned | `tests/test_collections_runtime.py` | `docs/api/collections.md` |
