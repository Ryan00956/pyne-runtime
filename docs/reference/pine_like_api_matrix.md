...
| Strategy risk | `strategy.risk.allow_entry_in(strategy.direction.long)`, `strategy.risk.max_drawdown(20, strategy.percent_of_equity)` | Partial | Drawdown risk is deterministic replay logic; it locks future entry/order submissions but is not broker liquidation or margin modeling | `tests/test_strategy_runtime.py` | `docs/api/strategy.md` |
