...
| Strategy risk | `strategy.risk.allow_entry_in(strategy.direction.long)`, `strategy.risk.max_drawdown(20, strategy.percent_of_equity)`, `strategy.risk.max_intraday_loss(5, strategy.cash)`, `strategy.risk.max_position_size(10)` | Partial | Risk rules are deterministic replay logic; drawdown locks globally, intraday loss resets at `session.isfirstbar`, max position size caps entries, and none model broker liquidation | `tests/test_strategy_runtime.py` | `docs/api/strategy.md` |
