"""Report TradingView external-capture status for TA golden fixtures."""
...
PRIORITY_FIXTURES = ["ta_core_indicators.json"]


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Report TradingView capture status for TA golden fixtures.",
    )
    parser.add_argument(
        "--golden-dir",
        type=Path,
    ...
    if args.missing_only:
        fixtures = [fixture for fixture in fixtures if fixture["status"] != "captured"]
...
def build_report(golden_dir: Path) -> dict[str, Any]:
    ...
    for fixture_path in sorted(golden_dir.glob(TA_FIXTURE_GLOB), key=_fixture_sort_key):
...
def print_table(report: dict[str, Any]) -> None:
    ...
    print()
    print(f"{'status':<13} {'priority':<8} {'fixture':<36} name")
    print("-" * 90)
    for fixture in report["fixtures"]:
        priority = "yes" if fixture["priority"] else "no"
        print(
            f"{fixture['status']:<13} {priority:<8} "
            f"{fixture['fixture']:<36} {fixture['name']}"
        )
